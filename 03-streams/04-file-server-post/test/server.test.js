  ...
  describe('тесты на файловый сервер', () => {
    ...
    after((done) => {
      ...
      fs_extra_1.default.writeFileSync(
        node_path_1.default.join(filesFolder, '.gitkeep'),
        ''
      );
    ...
    describe('POST', () => {
      it('возвращается ошибка 409 при создании файла, который есть', (done) => {
        fs_extra_1.default.copyFileSync(
          node_path_1.default.join(fixturesFolder, 'small.png'),
          node_path_1.default.join(filesFolder, 'small.png')
        );
        const mtime = fs_extra_1.default.statSync(
          node_path_1.default.join(filesFolder, 'small.png')
        ).mtime;
        const request = node_http_1.default.request(
          ...
          {method: 'POST'},
          (response) => {
            const newMtime = fs_extra_1.default.statSync(
              node_path_1.default.join(filesFolder, 'small.png')
            ).mtime;
            (0, chai_1.expect)(
              response.statusCode,
              'статус код ответа 409'
            ).to.equal(409);
            (0, chai_1.expect)(mtime, 'файл не должен перезаписываться').to.eql(
              newMtime
            );
            done();
          }
            ...
            setTimeout(() => {
              (0, chai_1.expect)(
                fs_extra_1.default.existsSync(
                  node_path_1.default.join(filesFolder, 'big.png')
                ),
      ...
      it('файл не должен оставаться на диске при обрыве соединения', (done) => {
        ...
        request.on('error', (err) => {
          ...
          setTimeout(() => {
            (0, chai_1.expect)(
              fs_extra_1.default.existsSync(
                node_path_1.default.join(filesFolder, 'example.txt')
              ),
